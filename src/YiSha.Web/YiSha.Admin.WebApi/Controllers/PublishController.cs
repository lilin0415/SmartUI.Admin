...
namespace YiSha.Admin.Web.WebApi.Controllers
{
    ...
    public class PublishController:BaseApiController
    {
        ...
        public async Task<TData<string>> GetDownloadUrl(string guid,string version)
        {
            ...
            return TData.CreateSuccessd(entity.FilePath);
        }
        ...
        /// <returns></returns>
        [HttpPost]
        public async Task<TData<string>> UploadProject(IFormCollection formData)
        {
            ...
            var service = new PublishedProjectService();
            var existed = await service.CheckExisted(model.ProjectGuid, model.AlignedVersion);
            if (existed)
            {
                return TData.CreateFailed($"项目:{model.Name}中已存在此版本:{model.Version}");
            }

            var files = formData.Files;
            if (files == null || files.Count != 1)
            {
                return TData.CreateFailed("请先选择文件并且只能选择一个文件");
            }
            ...
            {
                return TData.CreateFailed("文件格式错误");
            ...
            try
            {
                ...
                {
                    FileHelper.SafeDeleteFile(absoluteFileName);
                    return TData.CreateFailed($"上传文件(版本:{model.Version})校验失败，请重新上传");
                }
            ...
            {
                FileHelper.SafeDeleteFile(absoluteFileName);
                return TData.CreateFailed(ex.Message);
            }
        }
