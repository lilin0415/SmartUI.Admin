...
(function ($, ys) {
    ...
    $.extend(ys, {
        openDialog: function (option) {
            if (ys.isMobile()) {
                ...
                if (option.maxWidth) {
                    ...
                    console.log("FrameIndex", index);

                    if (index) {
                        //console.log("from parent");
                        //console.log(parent, $(parent).width());
                        //var parentWidth = $(parent).width();
                        //var currentWidth = $(window).width()-300;
                        //if (currentWidth < parentWidth) {
                        //    currentWidth = parentWidth;
                        //}
                        var parentWindow = window.parent;
                        
                        option.width = ($(parentWindow).width() - 300) + 'px';

                        //var w = $(window).width();
                        //option.width = w + 'px';

                        //如果未设置高度，高度也自动和父级高度一样
                        if (!option.height) {
                            //宽度最大化，高度也自动最大化
                            //加标题和footer高度
                            var h = $(window).height() + 42 + 55;
                            console.log("window height from index", h);
                    ...
                    else
                    {
                        ...
                        option.width = ($(window).width() - 300) + 'px';
                    }
                }
        ...
        msgError: function (content) {
            ...
            layer.msg(content, { icon: 2, time: 3000, shift: 5, width:"500px" });
        },
        ...
        ajax: function (option) {
            var opt = $.extend({
                ...
                error: function (xhr, status, obj) { ys.alertError(obj.message); },
        ...
        formatDate: function (v, format) {
            ...
            if (typeof v === 'string') {
                ...
                if (d === '1970-01-01 00:00:00' || d === '1970-01-01T00:00:00' || d ==='1970-01-01 00:00:00.000' || d === '0000-00-00 00:00:00') {
                    return "";
                }
                if (v.indexOf("/Date(") > -1)
        ...
        dateTimeFriendly: function (dateTimeStr) {
            if (this.isNullOrEmpty(dateTimeStr)) {
                return '';
            }
            //var date = '2015-03-05 17:59:00';
            var formatInputDateTimeStr = dateTimeStr.substring(0, 19);
            formatInputDateTimeStr = formatInputDateTimeStr.replace(/-/g, '/');
            //console.log(dateTimeStr);
            var inputTime = new Date(formatInputDateTimeStr);
            ...
            // 开始分解付入的时间
            var timeday = inputTime.getDate();
            var timehours = parseInt(inputTime.getHours());
            var timeminutes = inputTime.getMinutes();
            ...
            if (nowTime.getFullYear() !== inputTime.getFullYear()) {
                return dateTimeStr;
            }
            if (nowTime.getMonth() !== inputTime.getMonth()) {
                return dateTimeStr;
            }
