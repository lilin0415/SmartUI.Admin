...
(function($) {
    ...
    var periodsZhcn = ["分", "时", "天", "周", "月", "年"];
    for (var i = 0; i < periods.length; i++) {
        str_opt_period += "<option value='" + periods[i] + "'>" + periodsZhcn[i] + "</option>\n";
    }
    ...
    var methods = {
        init : function(opts) {
            ...
            block["mins"] = $("<span class='cron-block cron-block-mins'>"
                    ...
                    + "</select> 分minutes past the hour </span>")
                .appendTo(this)
                .data("root", this);
            ...
            block["time"] = $("<span class='cron-block cron-block-time'>"
                    + " 时间 <select name='cron-time-hour' class='cron-time-hour'>" + str_opt_hid
