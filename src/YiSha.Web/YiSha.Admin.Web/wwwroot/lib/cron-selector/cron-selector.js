(function($){
    ...
    timeNames = ['秒','分','时','天','月','周','年'],
    timeValues = ['0-59','0-59','0-23','1-31','1-12','1-7','2020-2049'],
    ...
    $.cronSelector = function(options){
        ...
        var renderPeriodHtml = function(timeType){
            ...
            $(timeContainer).append('<div class="pb-form-item">\n' +
                '                    <label class="pb-form-item">\n' +
                '                        <input class="' + typeSelectorClass + ' pb-form-radio" type="checkbox" name="' + selectorTypeEnum.period +'">周期</label> 从<input class="pb-form-input '+startInputClass+'" type="number" min="'+min+'" max="'+max+'">'+typeName+'开始,\n' +
        ...
        var renderAssignHtml = function(timeType){
            ...
            if(valueScope){
                ...
                '                    <label class="pb-form-item">\n' +
                    '                        <input class="' + typeSelectorClass +' pb-form-radio" type="checkbox" name="'+selectorTypeEnum.assign+'">指定时间' +
                '                    </label>'];
                for(var i = valueScope[0];i <= valueScope[1];i++){
                    timeValues.push('<label class="pb-form-item">\n' +
        ...
        var initPeriodValue = function(value,timeType){
            ...
            console.log(timeType, value, timeContainer, $('input[name="' + selectorTypeEnum.period + '"]', timeContainer));
        ...
        this.onChange = function (callback) {
            $('input.' + valueCheckBoxClass).change(function () {
                callback();

            });
            $('input.pb-form-input').change(function () {
                callback();

            });
            $('input.pb-form-radio').change(function () {
                callback();
