...
namespace YiSha.Admin.Web.Areas.SystemManage.Controllers
{
    ...
    public class DataDictDetailController : BaseController
    {
        ...
        public async Task<IActionResult> DataDictDetailIndex(string dictType)
        {
            var dataDictService = new DataDictService();
            var dictEntity = await dataDictService.GetEntityByType(dictType);

            this.ViewBag.DataDict= dictEntity;

            return View();
        }
        public async Task<IActionResult> DataDictDetailForm(string dictType)
        {
            var dataDictService = new DataDictService();
            var dictEntity = await dataDictService.GetEntityByType(dictType);
            this.ViewBag.DataDict = dictEntity;
