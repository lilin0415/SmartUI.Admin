...
namespace YiSha.Data.EF
{
    ...
    public class MySqlDatabase : IDatabase
    {
        ...
        public async Task BeginTrans( Func<DbHelper,Task>  action)
        {
            ...
            var trans = await dbConnection.BeginTransactionAsync();

            try
            {
                var reader = new DbHelper(null, dbConnection, trans);
                await action(reader);
        ...
        #region 数据源查询
        public async Task<DataSet> FindDataSet(string strSql)
        {
            using (var dbConnection = dbContext.Database.GetDbConnection())
            {
                var adapter = new MySqlDataAdapter(strSql, dbConnection as MySqlConnection);
                ...
                //var reader = await new DbHelper(dbContext, dbConnection).ExecuteReadeAsync(CommandType.Text, strSql, dbParameter) as MySqlDataReader;
