...
namespace YiSha.Data
{
    public static class DatabasesExtension
    {
        ...
        /// <param name="reader"></param>
        /// <returns></returns>
        public static List<T> IDataReaderToList<T>(IDataReader reader,string fieldName="")
        {
            //如果为基本类型，直接取值并转换
            if (typeof(T) == typeof(string) || typeof(T).IsValueType)
            {
                using (reader)
                {
                    ...
                    while (reader.Read())
                    {
                        var val = string.IsNullOrEmpty(fieldName) ? reader[0] : reader[fieldName];
                        if (!IsNullOrDBNull(val))
                        {
                            T convertedVal = default(T);
                            if (val is T)
                            {
                                convertedVal = (T)val;
                            }
                            else
                            {
                                ...
                                {
                                    convertedVal = (T)Convert.ChangeType(val, typeof(T));
                                }
                            }
                          
                            list.Add((T)convertedVal);
                        }
                        else
                        {
                            list.Add(default(T));
