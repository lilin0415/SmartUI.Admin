...
namespace YiSha.Data
{
    public class DbHelper
    {
        ...
        public DbHelper(DbContext _dbContext, DbConnection _dbConnection, IDbTransaction dbTasns =null)
        {
            dbContext = _dbContext;
            dbConnection = _dbConnection;
            dbCommand = dbConnection.CreateCommand();
            this.dbTasns = dbTasns;
        }
        #endregion
        ...
        private IDbTransaction dbTasns
        {
            get;set;
        }
        /// <summary>
        ...
        public async Task<IDataReader> ExecuteReadeAsync(CommandType cmdType, string strSql, params DbParameter[] dbParameter)
        {
            try
            {
                if (dbContext == null)
                {
                    PrepareCommand(dbConnection, dbCommand, this.dbTasns, cmdType, strSql, dbParameter);
                    if (this.dbTasns == null)
                    {
        ...
        public async Task<List<T>> FindList<T>(CommandType cmdType, string strSql, params DbParameter[] dbParameter)
        {
            var reader = await this.ExecuteReadeAsync(cmdType, strSql, dbParameter);
            var ret = DatabasesExtension.IDataReaderToList<T>(reader);
            reader.Close();
        ...
        /// <param name="dbParameter">Sql参数</param>
        /// <returns></returns>
        public async Task<int> ExecuteNonQueryAsync(CommandType cmdType, string strSql, params DbParameter[] dbParameter)
        {
            try
            {
                if (dbContext == null)
                ...
                else
                {
                    ...
                    if (logger != null)
                    {
                        obj = await logger.CommandNonQueryExecutedAsync(
                            connection,
                            dbCommand,
                            dbContext,
                            commandId,
        ...
        private void PrepareCommand(DbConnection conn, IDbCommand cmd, IDbTransaction isOpenTrans, CommandType cmdType, string strSql, params DbParameter[] dbParameter)
        {
