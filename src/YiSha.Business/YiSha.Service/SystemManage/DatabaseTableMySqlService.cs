...
using HtmlHelper = YiSha.Util.HtmlHelper;
...
namespace YiSha.Service.SystemManage
{
    public class DatabaseTableMySqlService : RepositoryFactory, IDatabaseTableService
    {
        ...
        public async Task<bool> RestoreDatabase(string database, string sqlFilePath)
        {
            string strSql = System.IO.File.ReadAllText(sqlFilePath, encoding: System.Text.Encoding.UTF8);
            var result = await this.BaseRepository().ExecuteBySql(strSql);
            return result > 0 ? true : false;
        }

        public async Task<bool> MakeInstallSql(string database)
        {
            string tablesDir = GlobalContext.GetAppData("install", "tables");
            FileHelper.DeleteDirectory(tablesDir);
            FileHelper.EnsureDirectory(tablesDir);

            bool succeed = false;
            var connStr = this.BaseRepository().db.dbContext.Database.GetConnectionString();
            using (var conn = new MySqlConnection(connStr))
            {
                using (MySqlCommand cmd = new MySqlCommand())
                {
                    using (MySqlBackup mb = new MySqlBackup(cmd))
                    {
                        ...
                        sNames.Add("caseexeclog");
                        sNames.Add("caseexecrecord");
                        sNames.Add("taskexecrecord");
                        sNames.Add("syslogapi");
                        sNames.Add("sysloglogin");
                        sNames.Add("syslogoperate");
                        mb.Database.GetDatabaseInfo(cmd, GetTotalRowsMethod.InformationSchema);

                        var exportCompleted = false;

                        mb.ExportCompleted += (ss, ee) => {
                            exportCompleted = true;
                        };

                        foreach (var t in mb.Database.Tables)
                        {
                            exportCompleted = false;

                            mb.ExportInfo.ExportTableStructure = true;
                            if (sNames.Contains(t.Name))
                            {
                            ...
                            mb.ExportInfo.TablesToBeExportedList = new List<string> { t.Name };
                            //mb.ExportInfo.TablesToBeExportedList.Add(t.Name);
                            var backupFile = FileHelper.Combine(tablesDir, t.Name + ".sql");
                            ...
                            SpinWait.SpinUntil(() => exportCompleted);
                        }
        ...
        public async Task<bool> InstallDatabase(string database)
        {
            string tablesDir = GlobalContext.GetAppData("install", "tables");
            ...
            using (var conn = new MySqlConnection(connStr))
            {
                using (MySqlCommand cmd = new MySqlCommand())
                {
                    using (MySqlBackup mb = new MySqlBackup(cmd))
                    {
                        ...
                        mb.ImportCompleted += (ss, ee) => {
                            importCompleted = true;
                        };
                        var tableSqls = Directory.GetFiles(tablesDir);
                        foreach (var tSql in tableSqls)
                        {
                            try
                            {
                                mb.ImportFromFile(tSql);
                            }
                            catch 
                            {
                                importCompleted = true;
                            ...
                            SpinWait.SpinUntil(() => importCompleted);
                        }
        ...
        public async Task<string> DatabaseBackup(string database)
        {
            ...
            //string strSql = string.Format(" backup database [{0}] to disk = '{1}'", database, backupFile);
            //var result = await this.BaseRepository().ExecuteBySql(strSql);
            return succeed ? backupFile : string.Empty;
        }
        public async Task<bool> RestoreFromBackup(string database,string backupFile)
        {
            var conn = this.BaseRepository().db.dbContext.Database.GetDbConnection();
            using (conn)
            {
