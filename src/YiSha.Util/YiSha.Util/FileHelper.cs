...
namespace YiSha.Util
{
    public class BizFileHelper
    {
        ...
        /// <returns></returns>
        public async static Task<TData<string>> UploadFile(UploadFileType fileModule, IFormFileCollection files)
        {
            ...
            switch (fileModule)
            {
                ...
                case UploadFileType.Project:
                    objCheck = CheckFileExtension(Path.GetExtension(file.FileName), ".sproj");
                    if (!objCheck.Status)
                    {
                        obj.Message = objCheck.Message;
                        return obj;
                    }
                    dirModule = UploadFileType.Project.ToString();
                    break;
                default:
