...
using BizFileHelper = YiSha.Util.BizFileHelper;
...
namespace YiSha.CodeGenerator.Template
{
    public class SingleTableTemplate
    {
        ...
        #region BuildEntity
        public string BuildEntity(BaseConfigModel baseConfigModel, DataTable dt,out string g)
        {
            var str = BuildEntity(baseConfigModel, dt, false);
            g = BuildEntity(baseConfigModel, dt, true);
            return str;
        }
        private string BuildEntity(BaseConfigModel baseConfigModel, DataTable dt, bool g)
        {
            ...
            sb.AppendLine("using Koo.Utilities.Helpers;");
            //using Koo.Utilities.Helpers;
            sb.AppendLine();
            ...
            if (g)
            {
                ...
                sb.AppendLine("    public partial class " + baseConfigModel.FileConfig.EntityName);
            }
            else
            {
            ...
            if (g)
            {
                ...
                sb.Append($"public static readonly string TblName = \"{baseConfigModel.TableName}\";");

                foreach (DataRow dr in dt.Rows)
                {
                    var TableIdentity = dr["TableIdentity"].ToString();
                    var Key = dr["Key"].ToString();
                    var column = dr["TableColumn"].ToString();
                    var IsNullable = dr["IsNullable"].ToString();
                    ...
                    var dbDatatype = dr["Datatype"].ToString();
                    var FieldDefault = dr["FieldDefault"].ToString();

                    var propertyType = TableMappingHelper.GetPropertyDatatype(dbDatatype);
                    ...
                    sb.AppendLine($"         [DbFieldAttribute(\"{column}\", \"{dbDatatype}\",\"{TableIdentity}\", \"{Key}\", \"{IsNullable}\", \"{FieldDefault}\")] ");
                    sb.AppendLine("         public " + propertyType + " " + column + " { get; set; }");
                }
            }
            else
            {
                sb.AppendLine($"        public {baseConfigModel.FileConfig.EntityName}()");
                sb.AppendLine("         {");
                sb.AppendLine();
                sb.AppendLine("         }");
            }
        ...
        public string BuildBusiness(BaseConfigModel baseConfigModel)
        {
            ...
            sb.AppendLine("            obj.Total = obj.Result.Count;");
            sb.AppendLine("            obj.Status = true;");
        ...
        public async Task<List<KeyValue>> CreateCode(BaseConfigModel baseConfigModel, string code)
        {
            ...
            JObject param = JsonHelper.ToJObject(code);
            ...
            if (!string.IsNullOrEmpty(param["CodeEntity"].ParseToString()))
            {
                ...
                {
                    BizFileHelper.CreateFile(codePath, codeEntity);
                    result.Add(new KeyValue { Key = "实体类", Value = codePath });
                }
            }
            if (!string.IsNullOrEmpty(param["CodeEntityG"].ParseToString()))
            {
                string codeEntityG = HttpUtility.HtmlDecode(param["CodeEntityG"].ToString());
                string codePath = Path.Combine(baseConfigModel.OutputConfig.OutputEntity, "YiSha.Entity", baseConfigModel.OutputConfig.OutputModule,"g", baseConfigModel.FileConfig.EntityName + ".g.cs");
                ...
                BizFileHelper.CreateFile(codePath, codeEntityG);
                result.Add(new KeyValue { Key = "实体类G", Value = codePath });
            }
            #endregion
            ...
            if (!param["CodeEntityParam"].IsEmpty())
            {
                ...
                if (!File.Exists(codePath))
                {
                    BizFileHelper.CreateFile(codePath, codeListEntity);
            ...
            if (!param["CodeService"].IsEmpty())
            {
                ...
                if (!File.Exists(codePath))
                {
                    BizFileHelper.CreateFile(codePath, codeService);
            ...
            if (!param["CodeBusiness"].IsEmpty())
            {
                ...
                if (!File.Exists(codePath))
                {
                    BizFileHelper.CreateFile(codePath, codeBusiness);
            ...
            if (!param["CodeController"].IsEmpty())
            {
                ...
                if (!File.Exists(codePath))
                {
                    BizFileHelper.CreateFile(codePath, codeController);
            ...
            if (!param["CodeIndex"].IsEmpty())
            {
                ...
                if (!File.Exists(codePath))
                {
                    BizFileHelper.CreateFile(codePath, codeIndex);
            ...
            if (!param["CodeForm"].IsEmpty())
            {
                ...
                if (!File.Exists(codePath))
                {
                    BizFileHelper.CreateFile(codePath, codeSave);
        ...
        private string GetBaseEntity(DataTable dt)
        {
            ...
            bool tenantId = columnList.Any(x => x == "TenantId");
            if (!id)
            {
                throw new Exception("数据库表必须有主键Id字段");
            }

            entity = tenantId ? "BaseTenantedEntity" : "BaseEntity";
            ...
            if (baseModifyTime && baseModifierId )
            {
                entity += ", IModifiableEntity";
            }

            if (baseIsDelete)
            {
                entity += ", IDeletableEntity";
            }

            if (baseVersion)
            {
                entity += ", IVersionableEntity";
            }
            return entity;
        }
